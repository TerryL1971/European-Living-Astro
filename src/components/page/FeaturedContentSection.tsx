...
//
// Ported from FeaturedContentSection.tsx. Two changes:
//
// 1. Reads the selected base from the shared nanostore ($selectedBase in
//    stores/baseStore.ts) instead of the dead contexts/BaseContext.tsx —
//    same pattern as every other re-enabled homepage section.
//
// 2. Queries the `featured_content` table directly via supabaseClient
//    instead of going through featuredContentService.ts, since that
//    service file wasn't available to port from. Matches an item if its
//    bases_served array contains either the selected base OR 'all' —
//    .overlaps() checks for any shared element between the two arrays,
//    which covers both cases in one query.
//
// The featured_content table already exists in production (confirmed via
// the live /admin/featured-content page actively serving real data) — no
// schema changes needed here, this only ports the read path.
  ...
  useEffect(() => {
    async function loadFeaturedContent() {
      ...
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('active', true)
        .overlaps('bases_served', [selectedBase, 'all'])
